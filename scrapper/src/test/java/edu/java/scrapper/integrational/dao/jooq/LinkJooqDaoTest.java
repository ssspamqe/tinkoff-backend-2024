...
public class LinkJooqDaoTest extends DatabaseIntegrationEnvironment {
    ...
    @Test
    void should_findByUrl() {
        saveLinkWithUrl("https://example.org");

        Optional<Link> actualLink = linkJdbcDao.findByUrl(URI.create("https://example.org"));

        assertThat(actualLink).isPresent();
    }

    @Test
    void should_beAbleToSave_when_noLinkWithSuchUrl() {
        Link link = linkJdbcDao.saveOrFindByUrl(URI.create("https://example.org"));
    ...
    @Test
    void should_beAbleToFind_when_linkWithSuchUrlExists() {
        saveLinkWithUrl("https://example.org");
    ...
    @Test
    void should_findAssociatedChatsByLinkId() {
        //Assert
        saveChatWithId(1);
        saveChatWithId(2);
        ...
        //Act
        Set<Long> associatedChatsIds = linkJdbcDao.findAssociatedChatsIdsByLinkId(linkId);
    ...
    @Test
    void should_updateLastChecked() {
        long linkId = saveLinkWithUrl("https://example.org");
        LocalDateTime expectedLastChecked = LocalDateTime.of(2000, 1, 1, 1, 1);

        linkJdbcDao.updateLastCheckedAtById(expectedLastChecked, linkId);
    ...
    @Test
    void should_findByLastCheckedAtBefore() {
        ...
        Set<String> actualLinkUrls = linkJdbcDao
